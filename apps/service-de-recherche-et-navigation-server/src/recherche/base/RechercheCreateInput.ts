/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  IsDate,
  ValidateNested,
} from "class-validator";
import { Type } from "class-transformer";
import { ItinRaireCreateNestedManyWithoutRecherchesInput } from "./ItinRaireCreateNestedManyWithoutRecherchesInput";

@InputType()
class RechercheCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  adresseId?: string | null;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  dateDeRecherche?: Date | null;

  @ApiProperty({
    required: false,
    type: () => ItinRaireCreateNestedManyWithoutRecherchesInput,
  })
  @ValidateNested()
  @Type(() => ItinRaireCreateNestedManyWithoutRecherchesInput)
  @IsOptional()
  @Field(() => ItinRaireCreateNestedManyWithoutRecherchesInput, {
    nullable: true,
  })
  itinRaires?: ItinRaireCreateNestedManyWithoutRecherchesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  termeDeRecherche?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  utilisateurId?: string | null;
}

export { RechercheCreateInput as RechercheCreateInput };
